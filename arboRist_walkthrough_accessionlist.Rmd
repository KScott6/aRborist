...
knitr::opts_chunk$set(error = FALSE, engine.opts='-l')
```

```{r run-options}
# main options
base_dir <- "/Users/scott/Documents/Projects/arboRist_walkthrough/test_accession_list" # change to whereever you want your project directory stored
project_name <- "ITS_test" # name your project
taxa_of_interest <- c("") # specify the taxa you want to grab accessions for
...
setup_project_structure <- function(base_dir,
                                      subdirs = c("intermediate_files", "metadata_files", "results_files", "temp_files", "multifastas",
                                                  "multifastas/aligned_fastas", "multigene_tree", "multigene_tree/prep", "single_gene_trees")) {
...
retrieve_ncbi_metadata <- function(project_name, custom_accession_path = NULL) {
  # specify - either using the arborist-created or custom path for the accession list
  # if a custom path is not specified, arborist will attempt to use the file labeled "all_pulled_accessions.csv" in the project's "intermediate_files" folder
  accession_file_path <- if (!is.null(custom_accession_path)) custom_accession_path else "./intermediate_files/all_pulled_accessions.csv"
  accession_list <- read.csv(accession_file_path, header = TRUE)
  ...
  for (i in 1:nrow(accession_list)) {
    tryCatch({
      ...
      # priting a small summary for the current accession metadata
  ...
  # writing metadata databases to file
...
```{r add-my-sequences}
# if warning on reading in custom file, add new line to end of custom metadata file (need to fix this)
merge_metadata_with_custom_file <- function(project_name) {
  # Check if my_lab_sequences is not set, then skip function
  if (is.null(my_lab_sequences) || my_lab_sequences == "") {
    cat("No custom sequence file path provided in 'my_lab_sequences'; skipping merge.\n")
  ...
  # Checking to make sure the custom file has the required columns
  ...
  # Merging the metadata with the custom sequences
...
curate_metadata <- function(project_name) {
  ...
  # Ensuring "gene", "product" and "accession_title" columns exist, creates them and fills with by creating them with NA values if missing
  required_columns <- c("gene", "product", "accession_title")
  for (col in required_columns) {
    if (!col %in% colnames(accession_list)) {
      accession_list[[col]] <- NA
    }
  }
...
filter_metadata <- function(project_name, taxa_of_interest, acc_to_exclude = NULL) {
  # if no taxa names provided (i.e. for projects that start with custom accession list), skip this curation step
  if (is.null(taxa_of_interest) || length(taxa_of_interest) == 0) {
    message("No 'taxa_of_interest' specified. Skipping function.")
    return(NULL)
  }
...
```{r process-single-region-fastas}
...
```{r}
# wrapper for all-in-one data retrieval functions (both accessions and metadata retrieval - i.e you are starting from a taxon name)
...
# Data Retrieval - starting from a name
```{r}
ncbi_data_fetch(project_name, max_acc_per_taxa, taxa_of_interest)
```

# Data Retrieval - starting from a list of accessions
```{r}
retrieve_ncbi_metadata(project_name, custom_accession_path = "/Users/scott/Documents/Projects/arboRist_walkthrough/test_accession_list/241031_SummaryFungi.csv")
```
...
# Run iqtree2 on single-gene multifasta files, with modelfinder option
