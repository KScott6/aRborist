...
# main options
base_dir <- "path.expand('~/Users/scott/Documents/Projects/EPF_tree/arborist_ncbi_metadata_pull')" # change to whereever you want your project directory stored
project_name <- "EMT_genome" # name your project

taxa_file <- read.csv("/Users/scott/Documents/Projects/EPF_tree/taxonomy_data/ncbi_taxonomy.csv", header = TRUE)
taxa_of_interest <- taxa_file$search.term

regions_to_include <- c("") # specify the regions you want to grab accessions for
...
# additional options
search_options <- "NOT (Contig[All Fields]) NOT (scaffold[All Fields]) NOT (genome[All Fields]) AND (biomol_genomic[PROP] AND ddbj_embl_genbank[filter]"
